              ...
              <Reveal key={s.title} delay={i * 0.06}>
                <Link to={s.to} className="group relative block h-[420px] overflow-hidden bg-emerald">
                  <img
                    ...
                    style={{ filter: "none" }}
                  />
                  <div className="absolute inset-x-0 bottom-0 h-[42%] bg-gradient-to-t from-[#6a5436]/20 to-transparent" />
                  <div className="service-card-copy relative mx-4 mb-4 flex flex-col justify-end p-5 md:mx-6 md:mb-6">
                    <span className="font-serif text-sm text-gold">{s.n}</span>
                    <h3 className="mt-2 font-serif text-2xl text-forest">{s.title}</h3>
                    <p className="mt-2 max-w-xs text-sm font-medium leading-relaxed text-charcoal/70">{s.desc}</p>
