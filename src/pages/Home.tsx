  ...
  return (
    <>
      ...
      <section className="hero-section relative flex h-[92vh] min-h-[640px] w-full items-end overflow-hidden bg-charcoal text-ivory md:h-screen">
        ...
        <div className="container-xl relative z-10 pb-20 pt-32 md:pb-24 md:pt-36">
          <div className="hero-copy fade-up max-w-[580px]">
            <p className="hero-eyebrow text-champagne" style={{ textShadow: "0 1px 8px rgba(0,0,0,0.7)" }}>
              SINCE 2008
            </p>
            <h1
              className="hero-headline mt-6 font-serif text-ivory"
            ...
            <p
              className="hero-description mt-7 text-ivory/80"
              style={{ animationDelay: "0.15s", textShadow: "0 1px 8px rgba(0,0,0,0.7)" }}
            >
      ...
      <section className="border-b border-champagne/30 bg-ivory">
        <div className="container-xl grid grid-cols-2 divide-y divide-champagne/30 md:grid-cols-3 md:divide-x md:divide-y-0">
          {[
            { value: <><CountUp end={5} decimals={1} />★</> },
            { value: <><CountUp end={business.yearsExperience} suffix="+" /></> },
            { value: "2008" },
          ].map((s, i) => (
            <div key={i} className="flex items-center justify-center py-10 text-center">
              <span className="font-serif text-4xl md:text-5xl" style={{ color: "#C9A24A" }}>{s.value}</span>
            </div>
          ))}
        </div>
      </section>
              ...
              <Reveal key={s.title} delay={i * 0.06}>
                <Link to={s.to} className="group relative block h-[420px] overflow-hidden bg-emerald">
                  <img
                    ...
                    style={{ filter: "none" }}
                  />
                  <div className="absolute inset-x-0 bottom-0 h-[55%] bg-gradient-to-t from-[#07110f]/35 via-[#07110f]/10 to-transparent" />
